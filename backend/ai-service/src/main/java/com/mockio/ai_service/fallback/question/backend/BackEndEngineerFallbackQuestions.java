    ...
    public static Map<InterviewDifficulty, List<FallbackQuestion>> byDifficulty() {
        ...
        map.put(InterviewDifficulty.EASY, List.of(
                new FallbackQuestion(
                        ...
                        List.of("REST", "HTTP", "API")
                ),
                new FallbackQuestion(
                        "HTTP 메서드",
                        "HTTP 메서드(GET, POST, PUT, DELETE)의 차이를 설명해 주세요.",
                        List.of("HTTP", "REST", "Method")
                ),
                new FallbackQuestion(
                        "HTTP 상태 코드",
                        "상태 코드 200, 400, 401, 403, 500의 의미를 각각 설명해 주세요.",
                        List.of("HTTP", "StatusCode", "REST")
                ),
                new FallbackQuestion(
                        "DTO와 Entity 분리",
                        "DTO와 Entity를 분리해서 사용하는 이유는 무엇인가요?",
                        List.of("DTO", "Entity", "Architecture")
                ),
                new FallbackQuestion(
                        "GET Body 금지 이유",
                        "GET 요청에 Body를 사용하지 않는 이유는 무엇인가요?",
                        List.of("HTTP", "GET", "REST")
                ),
                new FallbackQuestion(
                        "쿠키 vs 세션",
                        "쿠키와 세션의 차이를 설명해 주세요.",
                        List.of("Cookie", "Session", "Authentication")
                ),
                new FallbackQuestion(
                        "JPA Entity",
                        "JPA에서 @Entity는 어떤 역할을 하나요?",
                        List.of("JPA", "Entity", "ORM")
                ),
                new FallbackQuestion(
                        "트랜잭션 기초",
                        "트랜잭션(Transaction)이란 무엇인가요?",
                        List.of("Transaction", "ACID", "DB")
                ),
                new FallbackQuestion(
                        "JSON vs XML",
                        "JSON과 XML의 차이점은 무엇인가요?",
                        List.of("JSON", "XML", "Serialization")
                ),
                new FallbackQuestion(
                        "예외 처리 공통화",
                        "서버에서 예외 처리를 공통화하는 방법에는 어떤 것들이 있나요?",
                        List.of("Exception", "Spring", "API")
                )
        ));
        ...
        map.put(InterviewDifficulty.MEDIUM, List.of(
                ...
                new FallbackQuestion(
                        ...
                        List.of("JPA", "N+1", "FetchJoin")
                ),
                new FallbackQuestion(
                        "영속성 컨텍스트",
                        "영속성 컨텍스트의 역할과 장점은 무엇인가요?",
                        List.of("JPA", "PersistenceContext", "ORM")
                ),
                new FallbackQuestion(
                        "인덱스 원리",
                        "인덱스(Index)의 동작 원리와 성능에 미치는 영향을 설명해 주세요.",
                        List.of("DB", "Index", "Query")
                ),
                new FallbackQuestion(
                        "JWT 인증 흐름",
                ...
                new FallbackQuestion(
                        ...
                        List.of("JWT", "RefreshToken", "Security")
                ),
                new FallbackQuestion(
                        "동시성 문제",
                        "동시성 문제란 무엇이며, 어떤 상황에서 발생하나요?",
                        List.of("Concurrency", "RaceCondition", "DB")
                ),
                new FallbackQuestion(
                        "멱등성",
                        "REST API에서 멱등성(Idempotency)이 중요한 이유는 무엇인가요?",
                        List.of("REST", "Idempotency", "HTTP")
                ),
                new FallbackQuestion(
                        "@Transactional 범위",
                        "@Transactional이 적용되는 범위와 주의점은 무엇인가요?",
                        List.of("Spring", "Transaction", "Proxy")
                ),
                new FallbackQuestion(
                        "서버 페이징",
                        "페이징 처리를 서버에서 구현할 때 고려해야 할 사항은 무엇인가요?",
                        List.of("Paging", "DB", "Performance")
                )
        ));
